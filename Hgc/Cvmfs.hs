...
module Hgc.Cvmfs where
  ...
  transaction :: String -- ^ repository name
              -> IO ()
  transaction repo = safeSystem "cvmfs_server" ["transaction", repo] >>= \ex -> case ex of
    ExitSuccess -> return ()
    ExitFailure r -> ioError . userError $ "Cannot start transaction (exit code " ++ show r ++ ")."

  -- | Publish a CVMFS repository
  publish :: String -> IO ()
  publish repo = safeSystem "cvmfs_server" ["publish", repo] >>= \ex -> case ex of
    ExitSuccess -> return ()
  ...
  abort :: String -- ^ repository name
        -> IO ()
  abort repo = safeSystem "cvmfs_server" ["abort", "-f", repo] >>= \ex -> case ex of
    ExitSuccess -> return ()
